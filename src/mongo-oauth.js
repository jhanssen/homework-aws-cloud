...
var OAuthCodesSchema = new Schema({
...
mongoose.model('OAuthCodes', OAuthCodesSchema);
...
var OAuthAccessTokensModel = mongoose.model('OAuthAccessTokens'),
    ...
    OAuthCodesModel = mongoose.model('OAuthCodes');
...
model.saveAccessToken = function (token, clientId, expires, userId, callback) {
    console.log('in saveAccessToken (token: ' + token + ', clientId: ' + clientId + ', userId: ' + JSON.stringify(userId) + ', expires: ' + expires + ')');
...
model.getAuthCode = function (bearerCode, callback) {
    ...
    OAuthCodesModel.findOne({ authCode: bearerCode }, function(err, code) {
...
model.saveAuthCode = function (authCode, clientId, expires, user, callback) {
    console.log('in saveAuthCode (authCode: ' + authCode + ', clientId: ' + clientId + ', user: ' + JSON.stringify(user) + ', expires: ' + expires + ')');

    var code = new OAuthCodesModel({
        authCode: authCode,
        clientId: clientId,
        userId: user,
        expires: expires
...
model.saveRefreshToken = function (token, clientId, expires, userId, callback) {
    console.log('in saveRefreshToken (token: ' + token + ', clientId: ' + clientId +', userId: ' + JSON.stringify(userId) + ', expires: ' + expires + ')');
